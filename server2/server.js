...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'http/uploads/');  // donde se guardan los archivos
  },
...
app.use((req, res) => {
  try {
    ...
    const fullPath = path.join(__dirname, 'http', urlPath); // para hacer comprobaciones

    // ¿Nos está pidiendo algo de http/?
    if (!fullPath.startsWith(path.join(__dirname, 'http'))) {
