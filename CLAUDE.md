...
1. **Estado**: atributo en `Animaciones`, creado en `server/modules/ngsi.js` → `crear_animaciones()`.
   Atributos actuales: `anuncios`, `sponsors`, `infoteam`. Añadir infografía = añadir su atributo aquí.
2. **Overlay** (`public/js/<infografia>.js`): define `const ATTRIBUTE_NAME = "<attr>"`, hace
   ...
   el bloque (normalmente togglear una clase `.hidden`). Escucha `socket.on("message", ...)` y si el
   mensaje incluye `urn:ngsi-ld:Animaciones:` vuelve a llamar a `cogerAnimaciones()`.
   Requiere `<script src="/socket.io/socket.io.js">` en el HTML.
3. **Control** (`control.html` + `control.js`): togglea con `POST /v2/op/update`
   (`actionType:"update"`). `control.js` itera un array `CONTROLES = [{ attr, boton, estado }]`;
   añadir un control = una entrada ahí + `<button>`/`<span>` en `control.html`.
...
| mariadb | 3306 | MariaDB competition database (`acc26`) |
...
- Appends initial entities (universidades, equipos, rondas, anuncio, equipoMostrado, animaciones)
...
Special NGSI types with **no SQL table** (handled only in Orion): `Anuncio`, `EquipoMostrado`, `Animaciones`. The `ngsi.recv()` function explicitly skips sync for these.
...
| `GET /clima-data` | Proxy to external weather site |
